...
export const clientApi = {
  // Get all clients
  getAllClients: (auth: AuthContextProps | undefined = undefined) => {
    return api.get(CLIENT_ENDPOINT, auth);
  },
  
  // Get client by ID
  getClientById: (id: number, auth: AuthContextProps | undefined = undefined) => {
    return api.get(`${CLIENT_ENDPOINT}/${id}`, auth);
  },
  
  // Create a new client
  createClient: (client: Client, auth: AuthContextProps | undefined = undefined) => {
    return api.post(CLIENT_ENDPOINT, client, auth);
  },
  
  // Update an existing client
  updateClient: (id: number, client: Client, auth: AuthContextProps | undefined = undefined) => {
    return api.put(`${CLIENT_ENDPOINT}/${id}`, client, auth);
  },
  
  // Delete a client
  deleteClient: (id: number, auth: AuthContextProps | undefined = undefined) => {
    return api.delete(`${CLIENT_ENDPOINT}/${id}`, auth);
  },
  
  // Request image upload
  requestImageUpload: (id: number, auth: AuthContextProps | undefined = undefined) => {
